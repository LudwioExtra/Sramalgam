...
static std::unordered_map<uint32_t, CommandCallback> s_mCommands = {
	...
	AddCommand("splash_debug",
	{
		F::AimbotProjectile.m_iSplashDebug ^= 1;
		SDK::Output(F::AimbotProjectile.m_iSplashDebug
			? "Splash debug: ON  (green=valid, blue=bTime, magenta=rejected, red=trace)"
			: "Splash debug: OFF");
	})
};
