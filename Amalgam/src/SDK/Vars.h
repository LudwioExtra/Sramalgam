...
NAMESPACE_BEGIN(Vars)
	NAMESPACE_BEGIN(Menu)
		CVar(CheatTitle, "Cheat title", std::string(""), VISUAL | DROPDOWN_AUTOUPDATE);
		CVar(CheatTag, "Cheat tag", std::string("[CHEAT]"), VISUAL);
		...
		NAMESPACE_BEGIN(Theme)
			...
			CVar(Background, "Background color", Color_t(5, 5, 5, 255), VISUAL);
	...
	NAMESPACE_BEGIN(Colors)
		...
		CVar(AterisXhairBracket, "Ateris crosshair bracket", Color_t(0, 220, 80, 255), VISUAL);
		CVar(AterisXhairDot,     "Ateris crosshair dot",     Color_t(255, 60, 60, 255), VISUAL);
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		NAMESPACE_BEGIN(Projectile)
			...
			CVar(SplashPointsDirect, "Splash points direct", 100, SLIDER_MIN, 0, 3000, 5);
			CVar(SplashPointsArc, "Splash points arc", 100, SLIDER_MIN, 0, 3000, 5);
			CVar(SplashCountDirect, "Splash count direct", 100, NOSAVE | DEBUGVAR | SLIDER_MIN | SLIDER_PRECISION, 1, 3000, 5);
			CVar(SplashCountArc, "Splash count arc", 25, NOSAVE | DEBUGVAR | SLIDER_MIN | SLIDER_PRECISION, 1, 3000, 5);
			...
			CVar(SplashGroundBias, VA_LIST("Ground bias", "Push sphere sampling toward lower hemisphere for ground-level wallbangs"), 0.6f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 1.f, 0.05f);
			...
			// Fix #J: Old defaults [-0.7, 0.7] excluded walls directly behind the target (dot~-1).
			// New range [-1.0, 0.5] allows fully-behind walls, only excludes firmly player-facing points.
			CVar(Out2NormalMin, "Out2 normal min", -1.0f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, -1.f, 1.f, 0.1f);
			CVar(Out2NormalMax, "Out2 normal max", 0.5f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, -1.f, 1.f, 0.1f);
		...
		NAMESPACE_BEGIN(AterisDraw, Aimbot)
			CVar(SplashRadius, "Splash Damage Radius", false, VISUAL);
			CVar(RaySamples, VA_LIST("Ray samples", "Splash radius ray samples"), 64, VISUAL, 16, 256, 8);
			CVar(SphereSamples, "Sphere samples", false, VISUAL);
			CVarEnum(Crosshair, "Crosshair", 0, VISUAL, nullptr,
				VA_LIST("Off", "Ateris"),
				Off, Ateris);
			CVar(CrosshairSize,   VA_LIST("Size",   "Crosshair size"),   10, VISUAL | SLIDER_MIN, 4, 40);
			CVar(CrosshairGap,    VA_LIST("Gap",    "Crosshair gap"),    4,  VISUAL | SLIDER_MIN, 0, 20);
			CVar(CrosshairThick,  VA_LIST("Thick",  "Crosshair thickness"), 2, VISUAL | SLIDER_MIN, 1, 6);
		NAMESPACE_END(AterisDraw)
	...
	NAMESPACE_BEGIN(Visuals)
		...
		NAMESPACE_BEGIN(Simulation)
			...
			CVar(CustomTrails, "250e8 trails", false, VISUAL);
			CVar(ProjectileTrailMod, "Projectile trail", false, VISUAL);
			CVar(ExplosionEffectMod, "Explosion effect", false, VISUAL);
	...
	NAMESPACE_BEGIN(Misc)
		...
		NAMESPACE_BEGIN(Hitsound)
			CVar(Enabled, "Enabled", false);
			CVarEnum(Sound, "Sound", 0, NONE, nullptr,
				VA_LIST("None", "Aimbooster", "Ben", "Bonk", "Brotato", "Click", "Kick", "Money Claim", "Mouthsound", "Pop", "Quaver", "Regulus", "Satisfying Click", "Spiral Knight", "Tavern", "Ting", "Trident", "Water Drop", "Zelda", "XP Gain", "Bow Ding", "Button", "Egg Throw", "Minecraft", "Old Minecraft"),
				None, Aimbooster, Ben, Bonk, Brotato, Click, Kick, MoneyClaim, Mouthsound, Pop, Quaver, Regulus, SatisfyingClick, SpiralKnight, Tavern, Ting, Trident, WaterDrop, Zelda, XPGain, BowDing, Button, EggThrow, Minecraft, OldMinecraft);
			CVar(Volume, "Volume", 1.0f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 1.f, 0.05f);
		NAMESPACE_END(Hitsound)
	NAMESPACE_END(Misc)
