...
void CVisuals::DrawEffects()
{
	...
	for (auto& tSplash : G::AimbotSplashPoints)
	{
		if (tSplash.m_flTime < I::GlobalVars->curtime)
			continue;

		Color_t tColor = { 255, 255, 255, 255 };
		if (tSplash.m_iType & PointTypeEnum::In2) tColor = { 255, 255, 0, 255 };
		else if (tSplash.m_iType & PointTypeEnum::Out2) tColor = { 255, 0, 0, 255 };
		else if (tSplash.m_iType & PointTypeEnum::In) tColor = { 0, 255, 255, 255 };
		else if (tSplash.m_iType & PointTypeEnum::Out) tColor = { 0, 255, 0, 255 };

		// Draw as a small 3D wireframe box. Z-buffer=false so it renders through walls/floors
		H::Draw.RenderWireframeBox(tSplash.m_vOrigin, { -1, -1, -1 }, { 1, 1, 1 }, { 0, 0, 0 }, tColor, false);
	}
