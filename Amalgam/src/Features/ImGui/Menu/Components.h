...
namespace ImGui
{
	...
	inline void Divider(float flInset = H::Draw.Scale(), float flPrePadding = H::Draw.Scale(4), float flPostPadding = H::Draw.Scale(7), ImU32 uBorder = F::Render.Background2)
	{
		ImVec2 vSize = GetWindowSize();
	...
	inline void FTooltip(const char* sTooltip, bool bCondition = IsItemHovered(), float flWrapWidth = 300, ImFont* pFont = F::Render.FontSmall, ImVec2* pPosOverride = nullptr, ImVec2* pSizeOverride = nullptr)
	{
		if (bCondition)
		{
			...
			ImVec2 vP1 = { vPos.x + vText.x / 2 - H::Draw.Scale(4), vPos.y + H::Draw.Scale(1) - 0.5f }, vP2 = { vPos.x + vText.x / 2 + H::Draw.Scale(4), vPos.y + H::Draw.Scale(1) - 0.5f }, vP3 = { vPos.x + vText.x / 2, vPos.y - H::Draw.Scale(7) - 0.5f };
	...
	{
		PushStyleVar(ImGuiStyleVar_FramePadding, { H::Draw.Scale(4), H::Draw.Scale(4) });
	...
	inline bool Section(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false, uint32_t uHash = 0)
	{
		...
		PushStyleVar(ImGuiStyleVar_ItemSpacing, { H::Draw.Scale(4), 0 });
		if (sLabel[0] != '#')
		{
			ImVec2 vOriginalPos = GetCursorPos();
	...
	inline std::vector<WidgetWindow_t> WidgetTable(int iCount, float flHeight, std::vector<float> vWidths = {}, int iWindowFlags = ImGuiWindowFlags_None, int iChildFlags = ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_AlwaysUseWindowPadding)
	{
		...
		for (int i = 0; i < iCount; i++)
		{
			...
			vReturn.emplace_back(
				std::format("{}", i),
				ImVec2(flTotalWidth - GetStyle().WindowPadding.x / 2, GetCursorPos().y - H::Draw.Scale(4)),
	...
	inline bool FToggle(const char* sLabel, bool* pVar, int iFlags = FToggleEnum::None, bool* pHovered = nullptr)
	{
		...
		SetCursorPos({ vOriginalPos.x + 0.f, vOriginalPos.y + H::Draw.Scale(-5 + 9 * iWraps) });
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%g", int iFlags = FSliderEnum::None, bool* pHovered = nullptr)
	{
		...
		if (!bFull)
			vMins = { H::Draw.Scale(6), vSize.y - H::Draw.Scale(4) }, vMaxs = { vSize.x - H::Draw.Scale(6), vSize.y - H::Draw.Scale(6) };
#else
		ImVec2 vMins = { H::Draw.Scale(6), vSize.y - H::Draw.Scale(4) }, vMaxs = { vSize.x - H::Draw.Scale(6), vSize.y - H::Draw.Scale(6) };
#endif
		...
		if (pVar2)
		{
			...
			pDrawList->AddRectFilled({ vDrawPos.x + flLowerPos - H::Draw.Scale(3), vDrawPos.y + vMins.y - H::Draw.Scale(2) }, { vDrawPos.x + flLowerPos + H::Draw.Scale(3), vDrawPos.y + vMins.y + H::Draw.Scale(4) }, tAccent);
			pDrawList->AddRectFilled({ vDrawPos.x + flUpperPos - H::Draw.Scale(3), vDrawPos.y + vMins.y - H::Draw.Scale(2) }, { vDrawPos.x + flUpperPos + H::Draw.Scale(3), vDrawPos.y + vMins.y + H::Draw.Scale(4) }, tAccent);
		...
		else
		{
			...
			pDrawList->AddRectFilled({ vDrawPos.x + flPos - H::Draw.Scale(3), vDrawPos.y + vMins.y - H::Draw.Scale(2) }, { vDrawPos.x + flPos + H::Draw.Scale(3), vDrawPos.y + vMins.y + H::Draw.Scale(4) }, tAccent);
	...
	inline bool FDropdown(const char* sLabel, int* pVar, std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, const char* sDefaultPreview = "None", bool* pHovered = nullptr, int* pModified = nullptr)
	{
		...
		if (BeginComboPreview())
		{
			...
			if (bTitle)
			{
				...
				SetCursorPos({ vOriginalPos2.x + H::Draw.Scale(12), vOriginalPos2.y + H::Draw.Scale(4) });
	...
	inline bool ColorPicker(const char* sLabel, Color_t* pColor, bool bTooltip = true, ImVec2 vSize = { H::Draw.Scale(12), H::Draw.Scale(12) })
	{
		...
		PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, 0.f });
		PushStyleVar(ImGuiStyleVar_ItemInnerSpacing, { 0.f, 0 });
