...
private:
	...
	std::vector<Point_t> GetSplashPointsCached(Vec3 vOrigin, std::vector<std::pair<Vec3, int>>& vSpherePoints, int iSimTime);
	...
	void TracePoint(const Vec3& vPoint, int& iType, const Vec3& vTargetEye, const Info_t& tInfo, T& vPoints, F checkPoint, std::vector<DrawBox_t>* pBoxes = nullptr, std::vector<DrawLine_t>* pLines = nullptr, int i = 0);
	...
	bool TestAngle(const Vec3& vPoint, const Vec3& vAngles, int iSimTime, bool bSplash, bool bSecondTest = false, const std::vector<Vec3>* pEnemyPath = nullptr);

	bool HandlePoint(const Vec3& vOrigin, int iSimtime, float flPitch, float flYaw, float flTime, const Vec3& vPoint, const ProjectileInfo& tProjInfo, const MoveStorage& tMoveStorage, bool bSplash = false);
	bool HandleDirect(std::vector<Direct_t>& vDirectHistory, const ProjectileInfo& tProjInfo, const MoveStorage& tMoveStorage);
	bool HandleSplash(std::vector<Splash_t>& vSplashHistory, const ProjectileInfo& tProjInfo, const MoveStorage& tMoveStorage);
...
public:
	...
	// Read by AterisDisplay overlay every render frame.
	int    m_iResult    = false; // 1 = direct hit found, 2 = assistive
	Vec3   m_vPredicted = {};    // predicted body-origin of the target
	Info_t m_tInfo      = {};    // current solve info (incl. m_flRadius)
	int    m_iSplashDebug = 0;  // 0=off, 1=on — toggled by 'splash_debug' cmd
};
