...
void CMenu::DrawMenu()
{
	...
	if (!bSetPosition)
	{
		SetNextWindowPos((GetIO().DisplaySize - ImVec2(H::Draw.Scale(500), H::Draw.Scale(400))) / 2, ImGuiCond_FirstUseEver);
		SetNextWindowSize({ H::Draw.Scale(500), H::Draw.Scale(400) }, ImGuiCond_FirstUseEver);
		bSetPosition = true;
	}

	PushStyleVar(ImGuiStyleVar_WindowMinSize, { H::Draw.Scale(500), H::Draw.Scale(400) });
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar))
	{
		...
		float flSideSize = 100.f;

		// Pure black background and simple border (handled by ImGui style now, but let's ensure it's clean)
		pDrawList->AddRectFilled(vWindowPos, { vWindowPos.x + vWindowSize.x, vWindowPos.y + vWindowSize.y }, GetColorU32(ImGuiCol_WindowBg));
		pDrawList->AddRect(vWindowPos, { vWindowPos.x + vWindowSize.x, vWindowPos.y + vWindowSize.y }, GetColorU32(ImGuiCol_Border));
		...
		SetCursorPos({ H::Draw.Scale(8), vWindowSize.y - H::Draw.Scale(28) });
		FInputText("...", sSearch, H::Draw.Scale(84), ImGuiInputTextFlags_None);
			...
			TableNextColumn();
			{
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug Projectile"))
					{
						...
						FSlider(Vars::Aimbot::Projectile::SplashGroundBias);
	...
	case 1:
	{
		if (BeginTable("DrawTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Ateris Display"))
				{
					FToggle(Vars::Aimbot::AterisDraw::SplashRadius, FToggleEnum::Left);
					FToggle(Vars::Aimbot::AterisDraw::SphereSamples, FToggleEnum::Right);
					FSlider(Vars::Aimbot::AterisDraw::RaySamples);
					Divider();
					FDropdown(Vars::Aimbot::AterisDraw::Crosshair);
					PushTransparent(Vars::Aimbot::AterisDraw::Crosshair.Value == Vars::Aimbot::AterisDraw::CrosshairEnum::Off);
					{
						FColorPicker(Vars::Colors::AterisXhairBracket, FColorPickerEnum::Left);
						FColorPicker(Vars::Colors::AterisXhairDot,     FColorPickerEnum::Right);
						FSlider(Vars::Aimbot::AterisDraw::CrosshairSize,  FSliderEnum::Left);
						FSlider(Vars::Aimbot::AterisDraw::CrosshairGap,   FSliderEnum::Right);
						FSlider(Vars::Aimbot::AterisDraw::CrosshairThick, FSliderEnum::Left);
					}
					PopTransparent();
				} EndSection();
				if (Section("Moneybot Visuals"))
				{
					FToggle(Vars::Visuals::Simulation::ProjectileTrailMod, FToggleEnum::Left);
					FToggle(Vars::Visuals::Simulation::ExplosionEffectMod, FToggleEnum::Right);
				} EndSection();
			}
			/* Column 2 */
			TableNextColumn();
			{
				if (Section("Simulation"))
				{
					...
					FToggle(Vars::Visuals::Simulation::CustomTrails, FToggleEnum::Left);
	...
	case 0:
	{
		if (BeginTable("MiscTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Hitsound", 8))
				{
					FToggle(Vars::Misc::Hitsound::Enabled);
					FDropdown(Vars::Misc::Hitsound::Sound);
					FSlider(Vars::Misc::Hitsound::Volume);
				} EndSection();
			}
			EndTable();
		}
